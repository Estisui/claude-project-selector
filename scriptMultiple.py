...
ignore_file_types = ['.jpg', '.jpeg', '.png', '.gif', '.bmp', 
                     '.mp4', '.avi', '.mov', '.wmv', 
                     '.mp3', '.wav', '.ogg', 
                     '.ttf', '.otf', '.woff', '.woff2', 
                     '.svg']
...
def copy_files(source_folders, destination_folder):
    ...
    for source_folder in source_folders:
        for root, dirs, files in os.walk(source_folder):
            for file in files:
                # Skip .h files and ignored file types
                if file.endswith('.h') or any(file.endswith(ext) for ext in ignore_file_types):
...
def run_copy_files_and_js():
    ...
    if os.path.exists(config_file):
        with open(config_file, 'r') as file:
            ...
            source_folders = config.get('input_folders', [])
            destination_folder = config.get('output_folder', '')

            if source_folders and destination_folder:
                print(f"Copying files from {source_folders} to {destination_folder}")
